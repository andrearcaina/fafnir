...
import (
	"context"
	"fafnir/security-service/internal/api"
...
func main() {
	...
	if err := g.Wait(); err != nil {
		logger.Error(context.Background(), "Security service exited with error", "error", err)
		os.Exit(1)
	}

	logger.Info(context.Background(), "Security service exited cleanly")
}
